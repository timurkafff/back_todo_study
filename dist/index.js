...
app.use((0, cors_1.default)()); // Разрешаем все origins
app.use(express_1.default.json());
// Функция для чтения задач из файла
const readTasks = () => {
    const data = fs_1.default.readFileSync(TASKS_FILE, 'utf-8');
...
app.get('/api/tasks/not-completed', (req, res) => {
    const tasks = readTasks();
...
app.patch('/api/tasks/order', (req, res) => {
    const { ids } = req.body;
    const tasks = readTasks();
